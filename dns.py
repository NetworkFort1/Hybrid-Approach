...
try:
    ...
    # Initially set all as 'Normal'
    merged_log['label'] = 'Normal'

    # Label as 'Attack' if suspicious_domain is True
    if not weird_dns_log.empty:
        merged_log.loc[merged_log['suspicious_domain'], 'label'] = 'Attack'
    
    # Label as 'Attack' if protocol is other than udp
    merged_log.loc[merged_log['proto'] != 'udp', 'label'] = 'Attack'
    
    # Label as 'Attack' if query is not empty and dns_connection is False
    # or if query is 0 and dns_connection is False
    merged_log.loc[((merged_log['query'] != '') | (merged_log['query'] == 0)) & (merged_log['dns_connection'] == False), 'label'] = 'Attack'

    # Identify suspicious hosts based on the number of DNS queries exceeding the mean plus two standard deviations,
    # and label corresponding entries in the DataFrame as 'Attack'.
    dns_query_counts = merged_log.groupby(['id.orig_h', 'id.resp_h']).size()
    suspicious_hosts = dns_query_counts[dns_query_counts > dns_query_counts.mean() + 2 * dns_query_counts.std()].index
    merged_log.loc[merged_log[['id.orig_h', 'id.resp_h']].apply(tuple, axis=1).isin(suspicious_hosts), 'label'] = 'Attack'
    ...
    for idx, row in merged_log.iterrows():
        try:
            # Convert id.orig_h back to string before indexing
            row_dict = row.to_dict()
            es.index(index="dns-alerts", body=row_dict)
            print("DNS alert sent to Elasticsearch:", row_dict)  # Print statement
        except Exception as e:
            continue
