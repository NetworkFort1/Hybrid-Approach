    ...
    with subprocess.Popen(['tail', '-f', path], stdout=subprocess.PIPE, bufsize=1, universal_newlines=True) as proc:
        for line in proc.stdout:
            ...
            if line and line[0] != '#':
                ...
                # Rule-based labeling
                df['label'] = np.where(df['duration'] < 10, 'normal', '')
                df.loc[df['missed_bytes'] > 0, 'label'] = 'attack'

                # Threshold-based labeling
                df.loc[df['orig_pkts'] < 10, 'label'] = 'normal'
                df.loc[df['orig_ip_bytes'] > 1000, 'label'] = 'attack'
